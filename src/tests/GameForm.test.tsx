...
describe('GameForm Component', () => {
    ...
    test('can add a new rule', () => {
        ...
        const ruleNumberInputs = screen.getAllByLabelText(/Number/i);
        const ruleWordInputs = screen.getAllByLabelText(/Word/i);

        expect(ruleNumberInputs.length).toBe(2);
    ...
    test('submits the form and dispatches addGame', () => {
        ...
        fireEvent.change(screen.getAllByLabelText(/Number/i)[0], { target: { value: '3' } });
        fireEvent.change(screen.getAllByLabelText(/Word/i)[0], { target: { value: 'Fizz' } });
